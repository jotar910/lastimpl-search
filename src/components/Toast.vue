<template>
  <div class="fixed top-4 right-4">
    <div class="bg-red-600 shadow-lg mx-auto w-96 max-w-full text-sm pointer-events-auto bg-clip-padding rounded-lg block mb-3"
         ...
         v-for="alert of $store.getters['toast/toasts']" :key="alert">
      <div class="bg-red-600 flex justify-between items-center py-2 px-3 bg-clip-padding border-b border-red-500 rounded-t-lg">
        <p class="font-bold text-white flex items-center">
          <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="times-circle" class="w-4 h-4 mr-2 fill-current" role="img"
        ...
        <div class="flex items-center">
          <button type="button" @click="$store.commit('toast/remove', alert)"
            ...
            <fa-icon icon="xmark" class="text-white" />
          </button>
        </div>
      </div>
